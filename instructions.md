...
| WBS | **delete-all + insert** | 없음 | sortOrder 의존, 전체 교체 |
...
// CommunitySpec: communityType 동등 비교 + title/writer LIKE + projectId 동등 비교
...
### 커뮤니티 Entity 주의사항
- `content` 컬럼: `columnDefinition = "LONGTEXT"` 필수 (TEXT는 64KB 제한 → 웹에디터 이미지 저장 시 500 오류)
- `project` 필드: nullable FK — null이면 전체 공통 글, not null이면 해당 사업 전용 글
...
- [ ] 작성일 기본값: `dto.setPostDate(LocalDate.now())` (등록 폼 createForm)
- [ ] `CommunityDto.postDate`에 `@DateTimeFormat(pattern = "yyyy-MM-dd")` 선언
- [ ] `content` 컬럼 LONGTEXT (웹에디터 이미지 저장 위해 필수)
...
### 설계 원칙
- 목록 테이블에서 첨부파일 유무를 아이콘 1개로 표시 (파일 없으면 셀 비워둠)
- 파일 2개 이상이면 아이콘 우상단 빨간 배지로 수량 표시
- 아이콘 클릭 → 공통 모달에서 파일 목록 확인 + 클릭으로 다운로드
- `common.js` + `common.css`에 통합 — list.html에 data 속성만 추가하면 자동 동작
...
### REST API — GET /api/attachments
```
?entityType=ISSUE&ids=1,2,3
...
| 위험관리 | `RISK` |
| 요구사항 | `REQUIREMENT` |
| 보고관리 4종 | `CUSTOMER_REPORT` |
| 공지사항/자료실 | `COMMUNITY` |

> 자세한 내용은 `docs/22-list-attach-icons.md` 참조
